...
def get_questions(questions_num: int, db: Session) -> dict:
    while True:
        ...
        if check_question_is_in_database(question_ids, db):
            continue
...
def check_question_is_in_database(question_ids: list[int], db: Session) -> bool:
    for id in question_ids:
        query = select(QuestionAndAnswer).filter(QuestionAndAnswer.id == id)
